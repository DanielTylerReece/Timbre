...
@Gtk.Template(resource_path="/io/github/tylerreece/timbre/ui/widgets/lyrics_widget.ui")
class HTLyricsWidget(Gtk.Box, IDisconnectable):
    ...
    sync_indicator = Gtk.Template.Child()
    ...
    def set_lyrics(self, lyrics_text: str):
        ...
        # Subtle "synced" (time-stamped, auto-scrolling) vs "static" (plain
        # text, no follow) indicator in the top-right of the lyrics page.
        if self.has_timestamps:
            self.sync_indicator.set_label(_("Synced"))
            self.sync_indicator.set_tooltip_text(
                _("Time-synced lyrics — the current line follows playback")
            )
        else:
            self.sync_indicator.set_label(_("Static"))
            self.sync_indicator.set_tooltip_text(
                _("Plain lyrics — no time-sync available for this track")
            )
        self.sync_indicator.set_visible(True)
    ...
    def clear(self):
        ...
        self.sync_indicator.set_visible(False)
